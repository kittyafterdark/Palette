...
The picker treats both the compact widget and floating editor as Theme Studio-owned UI, so neither can accidentally become the selected page target. The compact widget is a persistent `document.body` portal and, where supported, a non-modal manual popover in the browser top layer so ordinary mobile drawer/tab stacking cannot cover it. Palette owns its fixed-position drag/persistence, opts this body-direct utility out of Lumiverse's global body-child UI scale, and keeps geometry in rendered viewport space so controls and pointer hitboxes stay aligned. Native reference catalogs stay hidden while the full editor is floated; they remain available in the normal drawer.
