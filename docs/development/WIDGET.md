...
The picker treats both the compact widget and floating editor as Palette-owned UI, so neither can accidentally become the selected page target. The compact widget has an explicit grab handle while collapsed and a draggable header while expanded; its last position is stored locally. Native reference catalogs stay hidden while the full editor is floated; they remain available in the normal drawer.
