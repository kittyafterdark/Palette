...
function cssValueTrace(value: unknown): Record<string, unknown> { return typeof value === 'string' ? { json: JSON.stringify(value), codePoints: [...value].map((character) => character.codePointAt(0) ?? 0) } : { json: JSON.stringify(value), type: typeof value } }
...
spindle.onFrontendMessage(async (payload, userId) => {
  ...
  try {
    if (payload.type === 'theme_studio:load_state') {
      const state = await spindle.userStorage.getJson<unknown>(STATE_PATH, { fallback: null, userId })
    ...
    if (payload.type === 'theme_studio:apply_theme_override') {
      ...
      await spindle.theme.apply({ variables: payload.variables as Record<string, string> }, userId)
