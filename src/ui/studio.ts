...
const GROUP_MEMBER_PACKETS: PacketType[] = ['background','pattern','text','typography','border','corners','spacing','shadow','glass','opacity','visibility','transform']
const GROUP_FRAME_PACKETS: PacketType[] = ['background','pattern','border','corners','shadow','glass','opacity']
...
function imageMaskMode(packet: ImagePacket): 'native' | 'none' | 'fade' | 'custom' { return packet.maskMode ?? (packet.fade.direction !== 'none' ? 'fade' : 'native') }
...
export class ThemeStudioUI {
  ...
  private renderPacket(override: ComponentOverride, packet: StylePacket): string {
    switch (packet.type) {
      ...
      case 'image': {
        ...
        const body = `${mediaControls}<section class="ts-magic-effects ts-mask-effects"><div class="ts-magic-head"><div><strong>Mask</strong><span>easy fade or advanced edge layers</span></div></div><div class="ts-segment ts-segment-four ts-mask-modes">${(['native','none','fade','custom'] as const).map((value) => `<button type="button" data-image-mask-mode="${value}" aria-pressed="${maskMode === value}">${value === 'native' ? 'Native' : value === 'none' ? 'None' : value === 'fade' ? 'Fade' : 'Custom'}</button>`).join('')}</div>${maskBody}</section><p class="ts-note">Want a color or gradient over the image? Use <strong>Front layer</strong> above, then add Background.${targetTag === 'img' ? ' If Front layer is unavailable, pick the image wrapper in the target ladder.' : ''}</p>`
  ...
  private projectSwatches(project: typeof this.store.snapshot.projects[number]): string[] {
    ...
    push(project.boost.primary.color); push(project.boost.secondary?.color)
  ...
  private bindDesign(root: ParentNode = this.root): void {
    ...
    root.querySelectorAll<HTMLButtonElement>('[data-image-mask-mode]').forEach((button) => button.addEventListener('click', () => this.updatePacket(button.closest<HTMLElement>('[data-packet-id]')?.dataset.packetId, (packet) => {
      ...
      const mode = button.dataset.imageMaskMode as NonNullable<ImagePacket['maskMode']>
...
${compileComponentOverride(draft, previewOptions)}`)
  ...
  private bindThemes(): void {
    ...
    const commitBoostColor = (key: 'primary' | 'secondary', value: string) => {
      if (!/^#[0-9a-f]{6}$/i.test(value)) return
    ...
    this.root.querySelectorAll<HTMLInputElement>('[data-boost-color]').forEach((input) => input.addEventListener('change', () => commitBoostColor(input.dataset.boostColor as 'primary' | 'secondary', input.value)))
    this.root.querySelectorAll<HTMLButtonElement>('[data-boost-recent]').forEach((button) => button.addEventListener('click', () => { const key = button.dataset.boostRecent as 'primary' | 'secondary', value = button.dataset.recentColor; if (value) commitBoostColor(key, value) }))
