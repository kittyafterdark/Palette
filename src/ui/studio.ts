...
export class ThemeStudioUI {
  ...
  private renderThemeSource(): string {
    ...
    const metadata = [source.name || source.archiveName || 'Imported .lumitheme', source.author ? `by ${source.author}` : ''].filter(Boolean).join(' · ')
    const components = Object.entries(source.components)
    const archiveSource = source.fidelity === 'archive'
    const fidelityLabel = archiveSource ? `Original archive source${source.archiveFormat ? ` · format ${source.archiveFormat}` : ''}` : 'Lumiverse-imported fallback'
