...
export class ThemeStudioUI {
  ...
  private renderThemeSource(): string {
    ...
    return `<section class="ts-section ts-theme-source"><div class="ts-code-label ts-theme-source-head"><div><p class="ts-kicker" style="margin:0">Theme Source</p><p class="ts-note">${escapeHtml(metadata || source.name || 'Imported .lumitheme')} · preserved separately from Palette Design.</p></div><div class="ts-source-actions"><span class="ts-chip">${editable ? 'Editable local fork' : 'Protected source'}</span><button class="ts-btn ${editable ? '' : 'ts-btn-primary'}" type="button" data-source-action="toggle-edit">${editable ? 'Protect source' : 'Fork & edit'}</button></div></div><div class="ts-code-label"><span class="ts-label">Global source CSS</span><span data-source-status class="${this.sourcePreviewResult.valid ? 'ts-status-ok' : 'ts-status-error'}">${this.sourcePreviewResult.valid ? 'Previewing' : escapeHtml(this.sourcePreviewResult.error ?? 'Invalid CSS')}</span></div><textarea class="ts-textarea ts-code ts-code-source" data-source-global-css ${editable ? '' : 'readonly'} spellcheck="false">${escapeHtml(source.globalCSS)}</textarea>${components.length ? `<div class="ts-source-components"><div class="ts-source-components-head"><strong>Native component source</strong><span>${components.length} section${components.length === 1 ? '' : 's'}</span></div>${componentMarkup}</div>` : componentMarkup}<p class="ts-note">Source loads first. Palette's visual Design layer overrides it without rewriting these rules. Unlocking only makes this project's local source snapshot editable.</p></section>`
  }
...
${compileComponentOverride(draft, previewOptions)}`)
  ...
  private bindCode(): void {
    const updateSourceStatus = () => {
      ...
      if (status) { status.className = this.sourcePreviewResult.valid ? 'ts-status-ok' : 'ts-status-error'; status.textContent = this.sourcePreviewResult.valid ? 'Previewing' : this.sourcePreviewResult.error ?? 'Invalid CSS' }
    }
  ...
  private async importNativeThemeFile(file: File): Promise<void> {
    ...
    try {
      ...
      this.nativeActionStatus = `Imported ${project.name} as an inert Palette project with protected Theme Source${imported.warnings.length ? ` · ${imported.warnings.length} native warning${imported.warnings.length === 1 ? '' : 's'}` : ''}. Nothing was applied.`
      await this.refreshAssets()
      this.workspace = 'code'
