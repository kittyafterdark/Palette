...
- Follow-up: restored the mini widget's utility-layer stacking above Lumiverse mobile tabs while keeping Spindle in sole control of its geometry. Palette now promotes only the native fixed `createFloatWidget()` shell's stacking order after the host mounts it; movement, clamping, persistence, pointer capture, and UI-scale math remain native.
