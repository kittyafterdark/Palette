...
describe('Phase Three persistence and migrations', () => {
  ...
  test('v44 preserves imported Theme Source separately and protects it until explicitly unlocked', () => {
    const state = createInitialState(); const project = state.projects[0]
    project.sourceTheme = {
      origin: 'lumitheme', editable: false, archiveName: 'Starry Elegant.lumitheme', name: 'Starry Elegant', author: 'Theme Author',
    ...
    expect(restored.version).toBe(44)
  ...
  test('schema v31 persists literal Generated Content intent', () => {
    ...
    expect(restored.version).toBe(44)
    expect(restored.projects[0].version).toBe(44)
  ...
  test('schema v35 promotes the saved SVG wardrobe to project-wide assets and sanitizes stored SVGs', () => {
    ...
    const restored = normalizeState(JSON.parse(JSON.stringify(state)))
    expect(restored.version).toBe(44); expect(restored.projects[0].version).toBe(44)
