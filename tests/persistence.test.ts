...
describe('Phase Three persistence and migrations', () => {
  ...
  test('v45 keeps imported Theme Source inert and protected until each behavior is explicitly enabled', () => {
    const state = createInitialState(); const project = state.projects[0]
    project.sourceTheme = {
      origin: 'lumitheme', editable: false, previewEnabled: false, archiveName: 'Starry Elegant.lumitheme', name: 'Starry Elegant', author: 'Theme Author',
      globalCSS: ':root { --source: hotpink; }',
      components: { BubbleMessage: { css: '.bubble { border-radius: 9px; }', enabled: true }, Composer: { css: '.composer { opacity: .8; }', enabled: false } },
    }
    project.customCss = '.user-override { color: white; }'
    const restored = normalizeState(JSON.parse(JSON.stringify(state)))
    expect(restored.version).toBe(45)
  ...
  test('schema v31 persists literal Generated Content intent', () => {
    ...
    expect(restored.version).toBe(45)
    expect(restored.projects[0].version).toBe(45)
  ...
  test('schema v35 promotes the saved SVG wardrobe to project-wide assets and sanitizes stored SVGs', () => {
    ...
    const restored = normalizeState(JSON.parse(JSON.stringify(state)))
    expect(restored.version).toBe(45); expect(restored.projects[0].version).toBe(45)
