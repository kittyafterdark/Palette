...
import { createInitialState, createStylePacket, normalizeComposerSvgSource, normalizeSvgTargetPath, themeSourcePreviewCss } from '../src/project/model'
...
describe('Phase Three persistence and migrations', () => {
  ...
  test('v46 keeps exact imported Theme Source inert and protected until each behavior is explicitly enabled', () => {
    const state = createInitialState(); const project = state.projects[0]
    project.sourceTheme = {
      origin: 'lumitheme', editable: false, previewEnabled: false, archiveName: 'Starry Elegant.lumitheme', fidelity: 'archive', archiveFormat: 3, name: 'Starry Elegant', author: 'Theme Author',
      archiveAssets: [{ slug: 'assets/star.png', archivePath: 'assets/001-star.png', mimeType: 'image/png', originalFilename: 'star.png' }],
      globalCSS: ':root { --source: hotpink; }',
      components: { BubbleMessage: { css: '.bubble { border-radius: 9px; }', tsx: 'export default null', enabled: true }, Composer: { css: '.composer { opacity: .8; }', enabled: false } },
    }
    project.customCss = '.user-override { color: white; }'
    const restored = normalizeState(JSON.parse(JSON.stringify(state)))
    expect(restored.version).toBe(46)
    expect(restored.projects[0].sourceTheme).toEqual(project.sourceTheme)
    expect(restored.projects[0].customCss).toBe('.user-override { color: white; }')
    expect(themeSourcePreviewCss(restored.projects[0].sourceTheme)).toBe('')

    const store = new ProjectStore(); store.hydrate(restored)
    store.setSourceGlobalCss(':root { --source: red; }')
    expect(store.activeProject.sourceTheme?.globalCSS).toContain('hotpink')
    store.setSourceThemePreviewEnabled(true)
    expect(themeSourcePreviewCss(store.activeProject.sourceTheme)).toContain('--source: hotpink')
    expect(themeSourcePreviewCss(store.activeProject.sourceTheme)).toContain('.bubble')
    expect(themeSourcePreviewCss(store.activeProject.sourceTheme)).not.toContain('.composer')
    store.setSourceThemeEditable(true)
    store.setSourceGlobalCss(':root { --source: red; }')
    store.setSourceComponentEnabled('Composer', true)
    store.setSourceComponentTsx('BubbleMessage', 'export default function Fork() {}')
    expect(store.activeProject.sourceTheme?.globalCSS).toContain('red')
    expect(store.activeProject.sourceTheme?.components.Composer.enabled).toBe(true)
    expect(store.activeProject.sourceTheme?.components.BubbleMessage.tsx).toContain('Fork')
  })

  test('migrates v44 Theme Source to preview-off instead of silently overlaying it on the active Lumiverse theme', () => {
    const state = createInitialState() as unknown as { version: number; projects: Array<Record<string, unknown>>; activeProjectId: string; savedStyles: unknown[] }
    state.version = 44
    state.projects[0].version = 44
    state.projects[0].sourceTheme = { origin: 'lumitheme', editable: true, globalCSS: '.legacy-source { color: pink; }', components: {} }
    const restored = normalizeState(state)
    expect(restored.version).toBe(46)
    expect(restored.projects[0].sourceTheme?.previewEnabled).toBe(false)
    expect(themeSourcePreviewCss(restored.projects[0].sourceTheme)).toBe('')
  })

  test('marks v45 Theme Source snapshots as host-draft fallback instead of claiming archive fidelity', () => {
    const state = createInitialState() as unknown as { version: number; projects: Array<Record<string, unknown>>; activeProjectId: string; savedStyles: unknown[] }
    state.version = 45
    state.projects[0].version = 45
    state.projects[0].sourceTheme = { origin: 'lumitheme', editable: false, previewEnabled: false, globalCSS: '/* @import stripped */', components: {} }
    const restored = normalizeState(state)
    expect(restored.version).toBe(46)
    expect(restored.projects[0].sourceTheme?.fidelity).toBe('host-draft')
  })
  ...
  test('schema v31 persists literal Generated Content intent', () => {
    ...
    expect(restored.version).toBe(46)
    expect(restored.projects[0].version).toBe(46)
  ...
  test('schema v35 promotes the saved SVG wardrobe to project-wide assets and sanitizes stored SVGs', () => {
    ...
    const restored = normalizeState(JSON.parse(JSON.stringify(state)))
    expect(restored.version).toBe(46); expect(restored.projects[0].version).toBe(46)
